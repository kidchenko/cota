...
//! registration and shows up under its name. Once the installer lands, set
//! `app_id` in the config to the shortcut's AUMID and the toasts get the right
//! name and icon.
...
/// icon by looking this string up against registered shortcuts, and an id with
/// no shortcut behind it shows nothing at all.
const APP_ID: &str = "kidchenko.Cota";

/// The shortcut is the registration. Windows has no API to ask "is this AUMID
/// known"; it simply declines to show the toast, silently, which is a miserable
/// thing to debug. So the check is for the artefact itself, and a portable copy
/// with no installer behind it falls back to borrowing PowerShell's identity —
/// wrong name on the toast, but a toast.
